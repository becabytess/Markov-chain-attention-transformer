"""
Experiment 1: Multi-Epoch Deep Convergence (5,000 Steps + Cosine Annealing LR Schedule)
Compares 1-Layer Gravimem (T=4 hops, 342k params) vs 4-Layer Standard Transformer (867k params)
on TinyShakespeare over ~75 full passes (5,000 steps).
"""
...
app = modal.App("gravimem-exp1-deep-convergence", image=image)


@app.function(gpu="T4", timeout=3600)
def run_deep_convergence():
    import math
    import time
    ...
    print("\n" + "=" * 80)
    print("  EXPERIMENT 1: DEEP CONVERGENCE SUMMARY TABLE")
    print("=" * 80)
    print(f"{'Model Architecture':<34} | {'Params':<9} | {'Train Loss':<10} | {'Val Loss':<8} | {'PPL':<6} | {'Overfit Gap':<11} | {'VRAM':<8}")
    print("-" * 96)
    for name, r in results.items():
        print(f"{name:<34} | {r['params']:<9,d} | {r['train_loss']:<10.4f} | {r['val_loss']:<8.4f} | {r['val_ppl']:<6.2f} | {r['overfit_gap']:<11.4f} | {r['peak_vram_mb']:<7.1f}M")
    print("=" * 80)
...
def main():
    print("Launching Experiment 1 (Deep Convergence) on dedicated Modal GPU...")
    res = run_deep_convergence.remote()
