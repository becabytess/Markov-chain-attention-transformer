"""
Test 2-Layer Gravimem (T=4 hops per layer) on Deep Nested Dyck-4 Grammar (Depth up to 30+)
Checks if stacking 2 physical Gravimem layers allows representation abstraction to solve nested bracket elimination.
"""
...
app = modal.App("gravimem-2layer-dyck", image=image)
...
def run_2layer_dyck():
    ...
    print("=" * 80)
    print("  TESTING 2-LAYER GRAVIMEM (T=4 HOPS PER LAYER) ON DYCK-4 GRAMMAR")
    print("=" * 80)
    ...
    class GravimemSurferBlock(nn.Module):
        ...
        def forward(self, x):
            ...
            # Residual + MLP block output
            out = s + self.mlp(self.ln2(s))
            return out

    class MultiLayerGravimem(nn.Module):
        def __init__(self, vocab_size, n_layers=2, T=4, d_model=128, d_mlp=512, max_len=256):
            super().__init__()
    ...
    print(f"\n---> Model: 2-Layer Gravimem (T={hops_T} Hops per Layer)")
    ...
    accs = {}
    print("\n" + "=" * 80)
    print(f"  2-LAYER GRAVIMEM (T={hops_T}) DYCK-4 FINAL EVALUATION RESULTS")
    print("=" * 80)
...
def main():
    print("Launching 2-Layer Gravimem on Dyck-4 on dedicated Modal GPU...")
    res = run_2layer_dyck.remote()
