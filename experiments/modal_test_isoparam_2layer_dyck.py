"""
Iso-Parameter Depth vs Capacity Experiment on Dyck-4 Grammar (Bracket Matching)
Tests if 2-Layer Gravimem constrained to the exact same parameter budget (~304k params)
as 1-Layer Gravimem maintains its ~80%+ accuracy gain.
"""
...
app = modal.App("gravimem-isoparam-2layer-dyck", image=image)


@app.function(gpu="T4", timeout=3600)
def run_isoparam_2layer_dyck():
    import math
    import random
    import time
    import torch
    ...
    print("=" * 80)
    print("  ISO-PARAMETER TEST: 2-LAYER GRAVIMEM (MATCHED TO ~304k PARAMS) ON DYCK-4")
    print("=" * 80)
    ...
    class MultiLayerGravimem(nn.Module):
        def __init__(self, vocab_size, n_layers=2, T=4, d_model=92, d_mlp=368, max_len=256):
            super().__init__()
    ...
    model = MultiLayerGravimem(vocab_size, n_layers=2, T=hops_T, d_model=d_model, d_mlp=d_mlp).to(device)
    params = sum(p.numel() for p in model.parameters())
    print(f"\n---> Model: Iso-Param 2-Layer Gravimem (d={d_model}, T={hops_T})")
    print(f"     Target Parameter Budget: ~304,000")
    print(f"     Actual Parameter Count : {params:,}")
    ...
    accs = {}
    print("\n" + "=" * 80)
    print(f"  ISO-PARAM 2-LAYER GRAVIMEM ({params:,} PARAMS) DYCK-4 RESULTS")
    print("=" * 80)
...
def main():
    print("Launching Iso-Param 2-Layer Gravimem on dedicated Modal GPU...")
    res = run_isoparam_2layer_dyck.remote()
    print("Run complete!")
