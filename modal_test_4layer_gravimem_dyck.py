"""
4-Layer Gravimem Experiment on Dyck-4 Grammar (Bracket Matching up to Depth 30+)
Parameter budget strictly capped below 800k (d=108, d_mlp=432 -> ~791k params)
vs 4-Layer Standard Transformer (830k params).
"""
...
app = modal.App("gravimem-4layer-dyck", image=image)
...
def run_4layer_dyck():
    ...
    print("=" * 80)
    print("  4-LAYER GRAVIMEM (BUDGET < 800k PARAMS) ON DEEP DYCK-4 GRAMMAR")
    print("=" * 80)
    ...
    class GravimemSurferBlock(nn.Module):
        def __init__(self, d_model, d_mlp, jump_offsets, T=4, max_len=256):
            super().__init__()
        ...
        def forward(self, x):
            ...
            out = s + self.mlp(self.ln2(s))
            return out

    class MultiLayerGravimem(nn.Module):
        def __init__(self, vocab_size, n_layers=4, T=4, d_model=108, d_mlp=432, max_len=256):
            super().__init__()
            self.tok_emb = nn.Embedding(vocab_size, d_model)
            self.pos_emb = nn.Embedding(max_len + 16, d_model)

            self.layers = nn.ModuleList([
                GravimemSurferBlock(d_model, d_mlp, jump_offsets, T=T, max_len=max_len)
    ...
    model = MultiLayerGravimem(vocab_size, n_layers=n_layers, T=hops_T, d_model=d_model, d_mlp=d_mlp).to(device)
    params = sum(p.numel() for p in model.parameters())
    print(f"\n---> Model: 4-Layer Gravimem (d={d_model}, T={hops_T} Hops/Layer)")
    print(f"     Parameter Budget Constraint: < 800,000")
    print(f"     Actual Model Parameter Count: {params:,}")
    assert params < 800000, f"Parameter budget exceeded! {params} >= 800000"
    ...
    accs = {}
    print("\n" + "=" * 80)
    print(f"  4-LAYER GRAVIMEM ({params:,} PARAMS) DYCK-4 FINAL EVALUATION")
    print("=" * 80)
...
def main():
    print("Launching 4-Layer Gravimem (<800k params) on dedicated Modal GPU...")
    res = run_4layer_dyck.remote()
    print("Run complete!")
